---
title: "Getting Started with dafJuliaWrapper"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting Started with dafJuliaWrapper}
...
[DataAxesFormats.jl](https://github.com/tanaylab/DataAxesFormats.jl), a Julia
package that provides a uniform interface for accessing multi-dimensional data
arranged along named axes. It generalizes AnnData-like functionality, supporting
scalars, 1D vectors, and 2D matrices organized along arbitrary named axes with
efficient memory management.

dafJuliaWrapper requires a working Julia installation (>= 1.10). Install Julia from
...
The `setup_daf()` call initializes the Julia runtime and ensures the required
Julia packages are available. This only needs to be called once per R session.
...
Axes are the named dimensions of your data. Each axis has a set of unique entry
names:
...
dafJuliaWrapper provides a composable query system for extracting and transforming data.
Queries are built using pipe operators:
...
The `[` operator provides a shorthand for `get_query()`:
...
- [DataAxesFormats.jl documentation](https://tanaylab.github.io/DataAxesFormats.jl/v0.2.0/)
  for the full Julia API reference
- [dafJuliaWrapper package documentation](https://tanaylab.github.io/dafJuliaWrapper/) for the
